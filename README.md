# KirimAja - Expedition Management System

KirimAja adalah platform manajemen ekspedisi modern berbasis fullstack yang dirancang untuk efisiensi pengiriman paket, mulai dari input di kasir hingga tracking real-time oleh pelanggan.
...
1.  **Clone Repositori:**
    ```bash
    git clone https://github.com/Fruzh/kirimaja.git
    cd kirimaja
    ```
...
4.  **Install Dependensi & Reset Database:**
    ```bash
    docker exec kirimaja_app composer install
    docker exec kirimaja_app php artisan migrate:fresh --seed --force
    ```
...
| :--- | :--- | :--- | :--- |
| **Admin** | Super Admin | `admin@kirimaja.id` | Pusat |
| **Manager** | Budi Santoso | `manager.jkt@kirimaja.id` | Jakarta |
| **Manager** | Sari Dewi | `manager.bdg@kirimaja.id` | Bandung |
| **Manager** | Hendra Wijaya | `manager.sby@kirimaja.id` | Surabaya |
| **Kasir** | Rina Kasir | `kasir1.jkt@kirimaja.id` | Jakarta |
| **Kasir** | Wati Kasir | `kasir1.bdg@kirimaja.id` | Bandung |
| **Kurir** | Zaki Kurir | `kurir1.jkt@kirimaja.id` | Jakarta |
| **Kurir** | Gilang Kurir | `kurir1.bdg@kirimaja.id` | Bandung |
