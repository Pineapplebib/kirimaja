...
<div class="max-w-xl">
    ...
    <x-ui.card class="p-6">
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5" id="userForm">
            @csrf @method('PUT')
            
            <x-form.input-group label="Nama Lengkap" name="name" :value="old('name', $user->name)" required />
            <x-form.input-group label="Email" name="email" type="email" :value="old('email', $user->email)" required />
            <x-form.input-group label="Password Baru" name="password" type="password" minlength="8" placeholder="Kosongkan jika tidak diubah" />
            ...
            <div id="vehicleFields" class="{{ $user->role !== 'courier' ? 'hidden' : '' }} space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200 transition-all">
                ...
                <x-form.select-dropdown 
                    ...
                    :selected="old('vehicle_id', $user->vehicle_id)"
                    searchable="true"
                />
            </div>
